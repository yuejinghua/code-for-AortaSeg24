...
class nnUNetPredictor(object):
    ...
    def predict_single_npy_array_corse(self, input_image: np.ndarray, image_properties: dict,
                                 segmentation_previous_stage: np.ndarray = None,
                                 output_file_truncated: str = None,
                                 save_or_return_probabilities: bool = False):
        """
    ...
    def predict_single_npy_array_fine(self, dct, input_image_corse: np.ndarray,
                                 output_file_truncated: str = None,
                                 save_or_return_probabilities: bool = False):
        """
        image_properties must only have a 'spacing' key!
        ...
        if self.verbose:
            print('predicting')
        input_image_corse = torch.from_numpy(input_image_corse)
        input_image_corse =input_image_corse.to(dct['data'].device)
        predicted_logits = self.predict_logits_from_preprocessed_data(torch.cat((dct['data'], input_image_corse), dim=0)).cpu()
...
if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(join(nnUNet_results, 'Dataset003_lungartery/nnUNetTrainerUME_GAG__nnUNetPlans__3d_fullres/'),
                                                   use_folds=(0,),
    ...
    for i in os.listdir(INPUT_PATH):
        image_name = os.path.join(INPUT_PATH,i)
        img, props = SimpleITKIO().read_images([image_name])
        ret, dic = predictor.predict_single_npy_array_corse(img, props, None, None, False)
        ...
        for num in range(num_features):
            roi_list = np.argwhere(labeled_array == num + 1)
            min_0 = roi_list[:, 0].min()
            if min_0 > max_18:
                continue
            else:
                min_2 = roi_list[:, 2].min()
                if min_2 < min_18:
                    pre_arr_updata[labeled_array == num + 1] = 22
                else:
                    pre_arr_updata[labeled_array == num + 1] = 23

        SimpleITKIO().write_seg(pre_arr_updata, os.path.join(OUTPUT_PATH,i), props)
        end_time = time.time()
